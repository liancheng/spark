...
private[kafka010] class KafkaSource(
  ...
  override def getBatch(start: Option[Offset], end: Offset): DataFrame = {
    ...
    val reuseKafkaConsumer = canReuseCachedConsumers(topicPartitions.length)

    // Create an RDD that reads from Kafka and get the (key, value) pair as byte arrays.
    val rdd = new KafkaSourceRDD(sc, executorKafkaParams, offsetRanges, pollTimeoutMs,
        failOnDataLoss, reuseKafkaConsumer).map { cr =>
      InternalRow(
        cr.key,
        cr.value,
  ...
  private def canReuseCachedConsumers(numTopicPartitions: Int): Boolean = {
    !kafkaReader.shouldDivvyUpLargePartitions(numTopicPartitions)
  }
  ...
  private def getOffsetRanges(
    ...
    val numPartitionsToRead = math.max(kafkaReader.minPartitions, topicPartitions.length)
    ...
    if (numPartitionsToRead == topicPartitions.length) {
      ...
      KafkaOffsetReader.divvyUpLargePartitions(offsets, numPartitionsToRead)
    }
  }
