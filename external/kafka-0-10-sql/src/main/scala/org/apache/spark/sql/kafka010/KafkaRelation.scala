...
private[kafka010] class KafkaRelation(
  ...
  override def buildScan(): RDD[Row] = {
    ...
    val offsetRanges: Array[KafkaSourceRDDOffsetRange] =
      if (kafkaReader.shouldDivvyUpLargePartitions(untilPartitionOffsets.keys.size)) {
        KafkaOffsetReader.divvyUpLargePartitions(offsetRangesBase.toSeq,
          kafkaReader.minPartitions).toArray
      } else {
