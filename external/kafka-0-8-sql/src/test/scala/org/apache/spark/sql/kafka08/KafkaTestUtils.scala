 ...
 *
 * The original file is `org.apache.spark.sql.kafka010.KafkaTestUtils`.
 */
class KafkaTestUtils extends Logging {
  ...
  def getAllTopicsAndPartitionSize(): Seq[(String, Int)] = {
    ZkUtils.getPartitionsForTopics(zkClient, ZkUtils.getAllTopics(zkClient)).mapValues(_.size).toSeq
  }
  ...
  def addPartitions(topic: String, partitions: Int): Unit = {
    AdminUtils.addPartitions(zkClient, topic, partitions)
    // wait until metadata is propagated
    (0 until partitions).foreach { p =>
      waitUntilMetadataIsPropagated(topic, p)
    }
  ...
  def sendMessages(topic: String, messages: Array[String], partition: Option[Int]): Unit = {
    val producer = new Producer[String, String](new ProducerConfig(producerConfiguration))
    try {
      producer.send(messages.map { m =>
        partition match {
          case Some(p) => new KeyedMessage[String, String](topic, null, partKey = p, m)
          case None => new KeyedMessage[String, String](topic, m)
        }
      }: _*)
    } finally {
      producer.close()
  ...
  def getLatestOffsets(topics: Set[String]): Map[TopicAndPartition, Long] = {
    val kafkaParams = new ju.HashMap[String, String]()
    kafkaParams.put("bootstrap.servers", brokerAddress)
  ...
  protected def brokerConfiguration: Properties = {
    ...
    // Disable auto topic creation. Otherwise asking a non-exist topic metadata will create the
    // topic and break our tests.
