...
class SparkSessionExtensions {
  ...
  type CatalogHooksBuilder = SparkSession => CatalogHooks

  private[this] val analyzerRuleBuilders = mutable.Buffer.empty[RuleBuilder]

  /**
   * Build the analyzer [[Rule]]s using the given [[SparkSession]].
   */
  private[sql] def buildAnalyzerRules(session: SparkSession): Seq[Rule[LogicalPlan]] = {
    analyzerRuleBuilders.map(_.apply(session))
  }

  /**
   * Inject an analyzer [[Rule]] builder into the [[SparkSession]]. These analyzer rules will be
   * executed as part of the resolution phase of analysis. An analyzer rule is used to transform
   * an (unresolved) LogicalPlan into a valid LogicalPlan.
   */
  def injectAnalyzerRule(builder: RuleBuilder): Unit = {
    analyzerRuleBuilders += builder
  }

  private[this] val checkAnalysisRuleBuilders = mutable.Buffer.empty[CheckRuleBuilder]

  /**
   * Build the check analysis [[Rule]]s using the given [[SparkSession]].
   */
  private[sql] def buildCheckAnalysisRules(session: SparkSession): Seq[LogicalPlan => Unit] = {
    checkAnalysisRuleBuilders.map(_.apply(session))
  }
   ...
   */
  def injectCheckAnalysisRule(builder: CheckRuleBuilder): Unit = {
    checkAnalysisRuleBuilders += builder
  }
   ...
   * executed during the last batch of optimization. An optimizer rule is used to improve the
  ...
  private[this] var catalogHooksOption: Option[CatalogHooksBuilder] = None

  private[sql] def buildCatalogHooks(session: SparkSession): Option[CatalogHooks] = {
    catalogHooksOption.map(_(session))
  }

  /**
   * Inject hooks for catalog related events into catalog shared between [[SparkSession]]s. The
   * hooks object is constructed using the first session that needs it. These hooks can be used to
   * track changes to permanent objects in the catalog. Only a single hooks object can be injected.
   * If a user tries to inject multiple hooks, only the last one is used.
   */
  def injectCatalogHooks(builder: CatalogHooksBuilder): Unit = {
    catalogHooksOption = Option(builder)
  }
}
