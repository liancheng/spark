...
import org.apache.spark.sql.catalog.{BaseCatalogHooks, CatalogHooks}
...
import org.apache.spark.sql.internal.HookCallingExternalCatalog
...
class SparkSessionExtensionSuite extends SparkFunSuite {
  ...
  private def checkExternalCatalog(session: SparkSession, expected: CatalogHooks): Unit = {
    session.sharedState.externalCatalog match {
      case catalog: HookCallingExternalCatalog =>
        assert(catalog.hooks == expected)
      case _ =>
        fail("Wrong catalog type")
    }
  }

  test("inject analyzer rule") {
    withSession(_.injectAnalyzerRule(MyRule)) { session =>
      assert(session.sessionState.analyzer.extendedResolutionRules.contains(MyRule(session)))
    }
  }

  test("inject check analysis rule") {
    withSession(_.injectCheckAnalysisRule(MyCheckRule)) { session =>
  ...
  test("inject spark planner strategy") {
    withSession(_.injectPlannerStrategy(MySparkStrategy)) { session =>
      assert(session.sessionState.planner.extraStrategies.contains(MySparkStrategy(session)))
    }
  }
  ...
  test("inject catalog hooks") {
    val extension = create { extensions =>
      extensions.injectCatalogHooks(_ => MyCatalogHooks)
    }
    withSession(extension) { session =>
      checkExternalCatalog(session, MyCatalogHooks)
    }
  }

  test("inject multiple catalog hooks objects") {
    val extension = create { extensions =>
      extensions.injectCatalogHooks(_ => MyCatalogHooks)
      extensions.injectCatalogHooks(_ => OtherCatalogHooks)
    }
    withSession(extension) { session =>
      checkExternalCatalog(session, OtherCatalogHooks)
    }
  }
...
case object MyCatalogHooks extends BaseCatalogHooks

case object OtherCatalogHooks extends BaseCatalogHooks

class MyExtensions extends (SparkSessionExtensions => Unit) {
  def apply(e: SparkSessionExtensions): Unit = {
    e.injectPlannerStrategy(MySparkStrategy)
    e.injectAnalyzerRule(MyRule)
  }
}
